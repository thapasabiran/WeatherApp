...
class FrontPageActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        vm.getCurrentWeatherSingle()?.observe(this) {
            ...
            binding.tempTextView.text = String.format( "%.1f° %s",currentWeather.temp, pref.getString("tempUnits", "C"))
            binding.cloudyText.text = currentWeather.long_description
            binding.LocationText.text = pref.getString("niceLocation", "") //get the saved location from preference
            Picasso.get().load("https://openweathermap.org/img/wn/" + currentWeather.icon + "@4x.png").into(binding.frontPageCloudImg)
        }
