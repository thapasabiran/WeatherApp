...
class JsonDbHelper {
    ...
    fun toCurrentWeather(json: String) : CurrentWeather{
        ...
        return CurrentWeather(
            ...
            short_description = current.getJSONArray("weather").getJSONObject(0).getString("main"),
            long_description = current.getJSONArray("weather").getJSONObject(0).getString("description"),
    ...
    fun toListHourlyWeather(json : String) : List<HourlyWeather>{
        ...
        (0 until hourly.length()).mapTo(list){
            HourlyWeather(
                ...
                hourly.getJSONObject(it).getJSONArray("weather").getJSONObject(0).getString("main"),
                hourly.getJSONObject(it).getJSONArray("weather").getJSONObject(0).getString("description"),
                hourly.getJSONObject(it).getJSONArray("weather").getJSONObject(0).getString("icon"),
    ...
    fun toListDailyWeather( json : String) : List<DailyWeather>{
        ...
        (0 until hourly.length()).mapTo(list){
            DailyWeather(
                ...
                hourly.getJSONObject(it).getInt("clouds"),
                hourly.getJSONObject(it).getDouble("pop"),
                if(hourly.getJSONObject(it).has("rain")) hourly.getJSONObject(it).getDouble("rain") else 0.0,
                if(hourly.getJSONObject(it).has("snow")) hourly.getJSONObject(it).getDouble("snow") else 0.0,
                hourly.getJSONObject(it).getDouble("uvi")
            )
        }
        return list
